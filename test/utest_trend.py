...
def test_trend_build_from_xml():
    ...
    trend = Trend(data=[data["SasData01"]], trend_axes={"entry": ["SASentry"]})
...
def test_trend_build_from_hdf5_with_multiple_axes():
    ...
    assert np.allclose(trend.get_trend_values("transmission")[0], trend.get_trend_values("transmission")[1])
    assert np.allclose(trend.get_trend_values("wavelength")[0], trend.get_trend_values("wavelength")[1])
...
def test_trend_build_interpolate(directory_name: str):
    ...
    data = ascii_reader.load_data(params)
    trend = Trend(data=data, trend_axes={"field": ["magnetic", "applied_magnetic_field"]})
...
def test_trend_q_axis_match():
    ...
    data = ascii_reader.load_data(params)
    trend = Trend(data=data, trend_axes={"index": ["magnetic", "counting_index"]})
