...
class Trend:
    ...
    def interpolate(self, axis: str) -> "Trend":
        ...
        new_trend = Trend(new_data,
                          self.trend_axes)
        return new_trend
||||||| parent of 280e6320 (Changes following review comments)
...
class Trend:
    ...
    def _filter_and_validate_data(self):
        ...
        # Warn about filtered items
        if invalid_indices:
            logger.warning(f"Warning: Removed data items at indices {invalid_indices} - not SasData objects or missing/invalid metadata")
    ...
    def _validate_manual_values(self):
        ...
        for axis_name, axis_config in self.trend_axes.items():
            if not isinstance(axis_config, list):
                raise ValueError(
                f"Manual values for axis '{axis_name}' should be passed as a list, "
                f"got {type(axis_config).__name__}"
            )
        if len(axis_config) == 0:
            raise ValueError(
                f"Manual values for axis '{axis_name}' must not be empty"
            )
        if not all(isinstance(v, (int, float)) for v in axis_config):
            raise ValueError(
                f"All values for axis '{axis_name}' must be numbers "
                f"(int or float)"
            )
        if len(axis_config) != len(self.data):
            raise ValueError(
    ...
    def interpolate(self, axis: str) -> "Trend":
        ...
        new_trend = Trend(new_data,
                          self.trend_axes)
        return new_trend
>>>>>>> 280e6320 (Changes following review comments)
