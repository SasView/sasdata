...
with StringIO() as fid:
    ...
    for unit_def in all_units:
        ...
        fid.write(
            f"{formatted_plural} = NamedUnit({unit_def.scale}, Dimensions({unit_def.length}, {unit_def.time}, {unit_def.mass}, {unit_def.current}, {unit_def.temperature}, {unit_def.moles_hint}, {unit_def.angle_hint}),"
        ...
        for mag in unit_def.magnitudes:
            ...
            fid.write(
                f"{combined_name_plural} = NamedUnit({combined_scale}, "
    ...
    for symbol, special_symbol, singular, plural, scale, _ in length_units:
        for prefix, power, name, unicode_suffix in [
            ...
            fid.write(
                f"{unit_name} = NamedUnit({scale**power}, Dimensions(length={power}), "
                f"name='{unit_name}', "
    ...
    for length_symbol, length_special_symbol, _, length_name, length_scale, _ in length_units:
        for time_symbol, time_special_symbol, time_name, _, time_scale, _ in time_units:
            ...
            fid.write(
                f"{accel_name} = NamedUnit({length_scale / time_scale**2}, "
