...
def deduce_qz(data: SasData):
    ...
    if data.abscissae.dimensionality == 2:
        # we start by making the approximation that qz=0
        qx = data.abscissae.axes[0]
        qy = data.abscissae.axes[1]
        qz = np.zeros_like(qx.value)

        # now check if metadata has wavelength information
        wavelength = getattr(
            getattr(getattr(getattr(data, "metadata", None), "instrument", None), "source", None), "wavelength", None
        )
